...
addIndicatorText = lambda name: "this.addIndicator('{i}', '{I}', this.settings.{I}{A});".format(i=name.lower(), I=name.upper(), A=IndicatorProperties[name]['input'])
class StrategyFileManager():
    def __init__(self, gekkoPath):
        ...
        for I in range(len(AllowedIndicators)):
            if not os.path.isfile("%s/indicators/%s.js" % (self.gekkoStratFolder, AllowedIndicators[I])):
    ...
    def checkStrategy(self, phenotype):
        ...
        if not Indicators:
            Indicators = sorted(FallbackIndicators, key=sortIndicators, reverse=True)[0:2]



        StrategyFileName = 'j' + ''.join(sorted(Indicators))

        if not StrategyFileName in self.sessionCreatedFiles or\
           not os.path.isfile("%s/%s" % (self.gekkoStratFolder, StrategyFileName)):
            self.createStrategyFile(Indicators, StrategyFileName)

        return StrategyFileName
    def createStrategyFile(self, Indicators, fname):
        ...
        FILE = open(self.gekkoStratFolder + fname +'.js', 'w')
        FILE.write(BASE)
        self.sessionCreatedFiles.append(fname)
        FILE.close()
