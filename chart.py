...
def dema(ax, price, params):
    emaslow, emafast, emadiff = moving_average_convergence(
        price, nslow=params["long"], nfast=params["short"]
    )
...
def macd(fig, ax, axis, price, params):
    ...
    signal = moving_average_convergence(
        emadiff, nslow=params["long"], nfast=params["short"]
    )
    ...
    ax_bot.hlines(
        [params["thresholds"]["down"], params["thresholds"]["up"]],
        axis[0],
        axis[-1],
        linestyles="dashed",
        label="thresholds({},{})".format(
            params["thresholds"]["down"], params["thresholds"]["up"]
        ),
    )
    ax_bot.legend()
